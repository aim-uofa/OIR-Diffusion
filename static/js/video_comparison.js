...
function playPics(imageId) {
    var imageMerge = document.getElementById(imageId + "Merge");
    ...
    var mergeContext = imageMerge.getContext("2d");
    ...
    imageMerge.addEventListener("mousemove", trackLocation, false); 
    imageMerge.addEventListener("touchstart", trackLocationTouch, false);
    imageMerge.addEventListener("touchmove", trackLocationTouch, false);

    function drawLoop() {
        mergeContext.drawImage(img, 0, 0, imgWidth, imgHeight, 0, 0, imgWidth, imgHeight);
        var colStart = (imgWidth * position).clamp(0.0, imgWidth);
        var colWidth = (imgWidth - (imgWidth * position)).clamp(0.0, imgWidth);
        mergeContext.drawImage(img, colStart + imgWidth, 0, colWidth, imgHeight, colStart, 0, colWidth, imgHeight);
        
        // ... (rest of the arrow drawing code remains unchanged)
...
function resizeAndShow(element) {
    var cv = document.getElementById(element.id + "Merge");
    cv.width = element.naturalWidth / 2;
    cv.height = element.naturalHeight;
    playPics(element.id);
}
